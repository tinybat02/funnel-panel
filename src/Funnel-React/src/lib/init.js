...
class FunnelGraph {
  ...
  getMainAxisPoints() {
    ...
    for (let i = 0; i <= size; i++) points.push(roundPoint((fullDimension * i) / size));
  ...
  addLabels() {
    ...
    this.percentages.forEach((percentage, index) => {
      ...
      if (percentage) {
        percentageValue.textContent = this.customPercents ? `${this.customPercents[index].toString()}%` : `${percentage.toString()}%`;
      }
